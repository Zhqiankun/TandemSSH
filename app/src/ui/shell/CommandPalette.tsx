...
function getSshActions(host: Host): {
  ...
  return [
    host.enableTerminal !== false && {
      ...
      icon: Terminal,
      get label() {
        return translateUiText("Terminal");
      },
    },
    ...
    host.enableTerminal !== false &&
      host.enableTmuxMonitor && {
        ...
        icon: Layers,
        get label() {
          return translateUiText("Tmux Monitor");
        },
      },
    host.enableTunnel && {
      type: "tunnel",
      icon: Network,
      get label() {
        return translateUiText("Tunnels");
      },
    },
    metricsEnabled && {
      type: "host-metrics",
      icon: Activity,
      get label() {
        return translateUiText("Host Metrics");
      },
    },
    host.enableProxmoxStats === true && {
      ...
      icon: HardDrive,
      get label() {
        return translateUiText("Proxmox Stats");
      },
    },
  ].filter(Boolean) as {
    type: TabType;
                          ...
                          <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                            ...
                            {canEditHost(host) && (
                              <>
                                <div className="w-px h-3.5 bg-border/60 mx-0.5 shrink-0" />
                                <button
                                  title={translateUiText("Edit Host")}
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    setIsOpen(false);
