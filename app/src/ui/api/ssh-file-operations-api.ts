...
export async function writeSSHFile(
  ...
  const response = await getFileManagerApiForSession(
    input.sessionId,
  ).post<SavedFileDocument>("/ssh/writeFile", input, { signal });
  invalidateCachedFileContent(input.sessionId, input.path);
  if (input.saveAs) invalidateCachedFileContent(input.sessionId, input.saveAs);
  return response.data;
}
