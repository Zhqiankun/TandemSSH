...
export function UserProfilePanel({
  ...
  const [language, setLanguage] = useState(() =>
    normalizeLanguageCode(localStorage.getItem(LANGUAGE_STORAGE_KEY)),
  );
  ...
  useEffect(() => {
    ...
    getVersionInfo()
      .then((info) => {
        ...
        setVersionStatus(
          /-(alpha|beta|rc)/i.test(info.localVersion ?? "")
            ? "beta"
            : info.status === "requires_update" || info.status === "beta"
              ? info.status
              : "up_to_date",
        );
  ...
  function resetToDefaults() {
    ...
    void changeAppLanguage(DEFAULT_LANGUAGE);
  ...
  function restoreLocalSnapshot() {
    ...
    const restoredLang = normalizeLanguageCode(
      restore(LANGUAGE_STORAGE_KEY, DEFAULT_LANGUAGE),
    );
  ...
  return (
    <div className="mx-auto flex w-full max-w-5xl flex-col gap-2 p-3">
      ...
      {isElectron() && (
        <section className="flex items-center justify-between gap-3 border border-border bg-card p-3">
          <div>
            <h3 className="text-xs font-medium">{t("tandem.mcp.title")}</h3>
            <p className="mt-1 text-[10px] leading-relaxed text-muted-foreground">
              {t("tandem.mcp.description")}
            </p>
          </div>
          <McpSettings />
        </section>
