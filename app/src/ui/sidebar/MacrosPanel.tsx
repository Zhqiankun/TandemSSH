...
export function MacrosPanel({
  ...
  const execute = useCallback(
    async (macro: TerminalMacro) => {
      ...
      try {
        mode = await chooseMode(macro.name);
      } catch {
        return;
      }
      ...
      try {
        await terminal.requestControlledTask(
          { kind: "macro", title: macro.name, steps: macro.steps },
          { signal: controller.signal, mode },
        );
        toast.info(t("tandem.legacy.taskCreated"));
      } catch (error) {
        if (!(error instanceof DOMException && error.name === "AbortError")) {
          toast.error(
            t("tandem.legacy.errors." + legacyErrorCode(error), {
              defaultValue: t("macros.failed"),
            }),
          );
        }
      } finally {
  ...
  const requestRun = (macro: TerminalMacro) => {
    if (!target?.terminalRef?.current?.requestControlledTask) {
      toast.error(t("macros.needTerminal"));
      return;
    }
    void execute(macro);
