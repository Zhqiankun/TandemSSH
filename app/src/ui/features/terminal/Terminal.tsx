  ...
  function SSHTerminal(
    ...
    useImperativeHandle(
      ...
      () => ({
        ...
        requestControlledTask: async (source, options) => {
          const sessionId = sessionIdRef.current;
          if (!sessionId || !isConnected || hostConfig.joinShareId)
            throw Error("SHARED_SESSION_REQUIRED");
          if (options?.signal?.aborted)
            throw new DOMException("Cancelled", "AbortError");
          const result = await requestLegacyTask(
            {
              sessionId,
          ...
          if (sessionIdRef.current === sessionId) {
            setRequestedTaskId(result.task.id);
    ...
    function setupWebSocketListeners(
      ...
      ws.addEventListener("message", (event) => {
        try {
          ...
          } else if (msg.type === "host_trust_pending") {
            if (connectionTimeoutRef.current) {
              clearTimeout(connectionTimeoutRef.current);
              connectionTimeoutRef.current = null;
            }
            window.dispatchEvent(new Event("host-trust-request"));
          ...
          } else if (msg.type === "collaboration.task.created") {
            if (msg.sessionId === sessionIdRef.current) {
              setRequestedTaskId(msg.taskId);
              setCollaborationOpen(true);
            }
    ...
    async function handleImageUpload(
      ...
      if (file.type && !file.type.startsWith("image/")) {
        toast.error(translateUiText("Choose an image file"));
        return;
      }
    ...
    async function handleClipboardImage() {
      ...
      try {
        ...
        toast.error(translateUiText("No image found in the clipboard"));
      } catch (error) {
    ...
    return (
      ...
      >
        ...
        <ConnectionScreen
          ...
          message={connectionError ?? t("terminal.connecting")}
