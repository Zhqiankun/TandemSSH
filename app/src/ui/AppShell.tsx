...
export function AppShell({
  ...
  const toggleAppFullscreen = useCallback(async () => {
    try {
      ...
      if (!document.fullscreenEnabled) {
        toast.error(
          translateUiText("Fullscreen is not supported by this browser"),
        );
        return;
      }
      ...
      toast.error(translateUiText("Unable to toggle fullscreen mode"));
    }
  }, []);
    ...
    try {
      ...
      if (restored.some((id) => id != null)) {
        ...
        const splitTab: Tab = {
          ...
          type: "split-screen",
          get label() {
            return translateUiText("Split #1");
          },
          openedAt: Date.now(),
      ...
      {!isMobile && (
        <>
          ...
          <Button
            ...
            title={translateUiText("Reset width")}
  ...
  useMcpSessionRequests(
    isElectron() && !!username && hostsLoaded && tabsReady,
    (hostId, instanceId) => {
      ...
      const host = allHosts.find((item) => item.id === String(hostId));
      if (!host) throw new Error("HOST_NOT_FOUND");
      openTab({ ...host, connectionOrigin: "local" }, "terminal", {
        instanceId,
        restoredSessionId: null,
      });
    },
          ...
          >
            {!isMobile && !sidebarOpen && (
              <button
                ...
                title={translateUiText("Open Sidebar")}
