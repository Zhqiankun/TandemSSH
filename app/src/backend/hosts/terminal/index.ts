  ...
  ws.on("message", async (msg: RawData) => {
    ...
    try {
      switch (type) {
        ...
        case "collaboration.snapshot":
        case "collaboration.takeover": {
          const session = sessionManager.getSession(currentSessionId);
          const participant = session
            ? sessionManager.getParticipantForWs(session, ws)
            : null;
          if (!session || session.userId !== userId || !participant?.isOwner) {
            ws.send(
              JSON.stringify({
                type: "collaboration.error",
                code: "CONTROL_BUSY",
          ...
          const state =
            type === "collaboration.takeover"
              ? session.control.takeover()
              : session.control.snapshot();
          ws.send(JSON.stringify({ type: "collaboration.snapshot", state }));
          break;
        }
    ...
    sshConn.on("error", (err: Error) => {
      ...
      ws.send(
        JSON.stringify({
          ...
          code: err.message.includes("Host denied (verification failed)")
            ? "HOST_TRUST_REJECTED"
            : undefined,
        }),
      );
