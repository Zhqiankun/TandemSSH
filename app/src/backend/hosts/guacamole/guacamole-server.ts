...
import { runtimePolicy, serviceListenOptions } from "../../runtime/policy.js";
...
const websocketOptions = {
  ...serviceListenOptions(GUAC_WS_PORT),
};
...
// Routes import this module too; disabled means no listener even on import.
let guacServer: GuacamoleLite | null = runtimePolicy.guacamole
  ? createGuacServer()
  : null;

export async function restartGuacServer(): Promise<void> {
  if (!runtimePolicy.guacamole) {
    throw new Error("Guacamole is disabled for this application process");
  }
  try {
    guacServer?.close();
  } catch (err) {
