...
export class HostRepository {
  ...
  readOwnedPolicyScope(
    userId: string,
    hostId: number,
  ): { folder: string; tags: string[]; identity: string } | null {
    if (this.context.dialect !== "sqlite")
      throw new Error("POLICY_STORE_UNSUPPORTED");
    const row = this.context.drizzle
      .select()
      .from(hosts)
      ...
      .get();
    if (!row) return null;
    const key = DataCrypto.getUserDataKey(userId);
    if (!key) throw new Error("DATA_EXPIRED");
    const host = DataCrypto.decryptRecord("ssh_data", row, userId, key);
    let tags: string[] = [];
    try {
      const parsed =
    ...
    return {
      folder: host.folder || "",
      tags,
      identity: `${host.username}@${host.ip}:${host.port || 22}`,
    };
  }
