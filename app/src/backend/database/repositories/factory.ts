...
export function createCurrentHostTrustRepository(): HostTrustRepository {
  if (
    needsExplicitPersist(resolveDatabaseDialect()) &&
    !DatabaseSaveTrigger.getStatus().initialized
  )
    throw Error("HOST_TRUST_STORAGE_UNAVAILABLE");
  return new HostTrustRepository(
    createCurrentRepositoryContext(),
    needsExplicitPersist(resolveDatabaseDialect())
      ? async () => {
          if (!DatabaseSaveTrigger.getStatus().initialized)
            throw Error("HOST_TRUST_STORAGE_UNAVAILABLE");
