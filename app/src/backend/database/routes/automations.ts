import {
  legacyAutomationExecutionEnabled,
  LEGACY_AUTOMATION_BLOCKED,
} from "../../../domain/commands/legacy-policy.js";
 ...
 *     responses:
 *       200:
 *         description: Reserved legacy response; execution is disabled in TandemSSH.
 *       404:
 *         description: Reserved legacy response.
 *       409:
 *         description: LEGACY_AUTOMATION_REQUIRES_MIGRATION. No execution or dry-run is started.
 */
router.post(
  "/:id/run",
  authenticateJWT,
  requireDataAccess,
  async (req: Request, res: Response) => {
    if (!legacyAutomationExecutionEnabled)
      return res.status(409).json({ error: LEGACY_AUTOMATION_BLOCKED });
    const userId = (req as AuthenticatedRequest).userId;
    const id = parseId(req.params.id);
    if (id === null) return res.status(400).json({ error: "Invalid id" });
 ...
 *         description: Reserved legacy response.
 *       409:
 *         description: LEGACY_AUTOMATION_REQUIRES_MIGRATION. No token lookup or execution is performed.
 */
