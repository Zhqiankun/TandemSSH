import { notifyAiAccessChanged } from "../../ai/access-events.js";
import { isAiGloballyEnabled } from "../../ai/gating.js";
...
export function registerUserSettingsRoutes(
  ...
  router.get("/ai-enabled", authenticateJWT, async (_req, res) => {
    try {
      res.json({
        enabled: await isAiGloballyEnabled(),
      });
    } catch (err) {
  ...
  router.patch("/ai-enabled", authenticateJWT, async (req, res) => {
    ...
    try {
      ...
      if(!enabled)notifyAiAccessChanged({enabled:false});
      await createCurrentSettingsRepository().set(
        "ai_globally_enabled",
        enabled ? "true" : "false",
      );
      if(enabled)notifyAiAccessChanged({enabled:true});
