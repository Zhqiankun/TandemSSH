...
describe("legacy automation execution routes", () => {
  it.each([false, true])(
    "blocks manual execution, including dryRun=%s, before invoking the engine",
    async (dryRun) => {
      const res = await invoke("post", "/:id/run", {
        params: { id: "1" },
        body: { dryRun },
      });
  ...
  it.each(["a".repeat(64), "short"])(
    "does not resolve or execute legacy webhook tokens (%s)",
    async (token) => {
      ...
      expect(repository.listAllEnabled).not.toHaveBeenCalled();
      expect(run).not.toHaveBeenCalled();
    },
  );
});
