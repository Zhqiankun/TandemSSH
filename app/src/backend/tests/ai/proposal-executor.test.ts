...
describe("applyProposal", () => {
  ...
  it.each([
    { host: null, code: 0, command: "uptime" },
    { host: { id: 9, ip: "10.0.0.9" }, code: 0, command: "uptime" },
    { host: { id: 9, ip: "10.0.0.9" }, code: null, command: "pwd" },
    { host: { id: 9, ip: "10.0.0.9" }, code: 1, command: "cat /etc/shadow" },
  ])(
    "refuses legacy command proposals without opening a separate SSH channel: %j",
    async ({ host, code, command }) => {
      resolveHostById.mockResolvedValue(host);
      execCommand.mockResolvedValue({ stdout: "old", stderr: "", code });
      await expect(
        applyProposal("propose_run_command", { hostId: 9, command }, "user-1"),
      ).rejects.toThrow("SHARED_SESSION_REQUIRED");
