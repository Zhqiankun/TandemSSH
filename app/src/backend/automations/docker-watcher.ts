import { legacyAutomationExecutionEnabled } from "../../domain/commands/legacy-policy.js";
...
export async function listDockerWatchedHosts(): Promise<Map<number, string>> {
  if (!legacyAutomationExecutionEnabled) return new Map();
...
export async function pollDockerEvents(
  ...
  if (!legacyAutomationExecutionEnabled) return;
