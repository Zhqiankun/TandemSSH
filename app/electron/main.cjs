...
(function setupPortableDataDir() {
  ...
  const envOverride = process.env.TANDEMSSH_DATA_DIR;
...
const updateService = require("./update-ipc.cjs").registerUpdateIpc({
  app,
  ipcMain,
  dialog,
  shell,
  ...
  isDev,
  beforeInstall: async () => {
    await downloadIpc.cancelActive();
  },
});
...
// TandemSSH has an independent release channel. An upstream Termix release
// must never be offered as an update to this derived application.
ipcMain.handle("check-electron-update", () => ({
  success: true,
  status: "update_check_disabled",
  localVersion: updateService.snapshot().currentVersion,
}));
