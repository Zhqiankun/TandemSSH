...
- 修复 alpha.2 文件列表遗漏 setuid/setgid/sticky 的问题，避免权限对话框漏掉已有特殊位。真实 Windows 属性对话框与 Linux 2755 → 0755 读回验证通过。
- 新增中文“所有者/组”编辑，使用数字 UID/GID、远端账户实际权限和执行后读回确认；独立保存，不自动提权或重试。选择链接时修改链接自身。
- 修复符号链接路径的重复 URL 解码与换行截断，改用 SFTP 查询完整目标；断链、循环链接和特殊文件目标返回失败。
- 保留历史脱敏导出、执行来源与策略摘要，以及启动/每 20 分钟更新检测。

相关实现和实机证据见 docs/80-unix-permissions.md、docs/81-symlink-paths.md、docs/84-file-ownership.md。此版本仍为开发预览，不代表全部产品验收完成。alpha.1/alpha.2 安装版可在软件内检查新版；alpha.0 需手动安装一次当前版本。
