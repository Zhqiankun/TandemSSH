...
已接入显式保存本机加密草稿、保存草稿并关闭、重新检查、审阅恢复和删除入口。默认编辑仍在内存，保存采用系统凭据库密钥的 AES-256-GCM 全快照加密；恢复使用新连接/文档版本，比较原文、当前远端和草稿，不自动上传。实际 Windows 桌面完成保存、退出、外部修改远端、重启和恢复，远端内容保持不变。具体身份边界、并发处理、失败证据和未完成的迁移/离线管理见 [第 38 份文档](38-encrypted-file-drafts.md)。
