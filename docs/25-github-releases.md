...
2026-09-10，用户确认从 GitHub 下载试用。Release 工作流现分别处理稳定 vX.Y.Z 和预览 vX.Y.Z-alpha.N 标签。validate-release-ref.cjs 负责严格版本格式、package.json 版本和 tag/HEAD 一致性；工作流只编排现有完整检查、标准原生重编译、真实安装/升级/卸载与发布。应用业务模块不依赖发布脚本，稳定更新清单生成契约保持不变。
