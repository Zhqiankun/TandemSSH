...
[下载安装版 EXE](https://github.com/Zhqiankun/TandemSSH/releases/download/v0.1.0-alpha.12/TandemSSH-0.1.0-alpha.12-x64.exe) · [下载免安装 ZIP](https://github.com/Zhqiankun/TandemSSH/releases/download/v0.1.0-alpha.12/TandemSSH-0.1.0-alpha.12-x64.zip) · [发布说明与校验和](https://github.com/Zhqiankun/TandemSSH/releases/tag/v0.1.0-alpha.12)

当前为 Windows x64 的 0.1.0-alpha.12 开发预览，建议先在测试服务器体验。EXE 直接安装；ZIP 完整解压后运行 TandemSSH.exe。GitHub 自动附带的 Source code 是源码包。完整产品验收仍在进行，具体边界见[预览说明](docs/75-preview-release-notes.md)。

## 仓库与版本更新

源码仓库：[Zhqiankun/TandemSSH](https://github.com/Zhqiankun/TandemSSH)。当前为开发预览，完整功能清单仍在实施。标准 Windows 构建、安装、中文启动及卸载保留数据已通过 CI；已公开 alpha.12 Release（包含旧终端启动输入修复、模型 Key 字段加密修复、目录联动和主机导入确认，保留 20 分钟更新检查），真实 NSIS 跨版本升级和数据保留已在隔离 CI 环境验证；完整历史迁移和公开稳定更新渠道仍待验收。
...
保留的 alpha.4 开发目录客户端已通过真实 GitHub 源检测到 alpha.12；公开 EXE、ZIP、blockmap 和清单均已实际下载并校验，发布及验证范围见 [alpha.12 发行记录](docs/206-alpha12-release.md)。

## 配置备份与恢复

在桌面“用户资料 → 数据”中预览并导出配置，或选择备份文件、预览后确认导入。当前支持 SSH 主机基本信息、命令流程、外观、界面偏好和自定义快捷键。凭据与流程主机需重新配置；快捷键先禁用，编辑确认后启用，片段需重新选择。导入不会自动执行。格式范围与真实桌面验证见[配置备份文档](docs/54-configuration-backup.md)和[应用设置备份](docs/55-application-settings-backup.md)。
