...
源码仓库：[Zhqiankun/TandemSSH](https://github.com/Zhqiankun/TandemSSH)。当前为开发预览，完整功能清单仍在实施。标准 Windows 构建、安装、中文启动及卸载保留数据已通过 CI；已公开 alpha.4 Release（包含编辑器边界、文件时间汉化、MCP 版本同步和随包依赖声明，保留 20 分钟更新检查），真实 NSIS 跨版本升级和数据保留已在隔离 CI 环境验证；完整历史迁移和公开稳定更新渠道仍待验收。
...
alpha.3 客户端已通过真实 GitHub 源检测到 alpha.4；发布及验证范围见 [alpha.4 发行记录](docs/96-alpha4-release.md)。
